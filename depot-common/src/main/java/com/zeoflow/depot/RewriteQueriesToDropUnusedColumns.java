...
/**
 * When present, {@link RewriteQueriesToDropUnusedColumns} annotation will cause Depot to
 ...
 * </pre>
 * <p>
 * Normally, Depot would print a {@link DepotWarnings#CURSOR_MISMATCH} warning since the query result
 * has additional columns that are not used in the response. You can annotate the method with
 * {@link RewriteQueriesToDropUnusedColumns} to inform Depot to rewrite your query at compile time to
 ...
 * }
 * </pre>
 * At compile time, Depot will convert this query to {@code SELECT name, lastName FROM (SELECT *
 ...
 * respectively.
 * <p>
 * Note that Depot will not rewrite the query if it has multiple columns that have the same name as
