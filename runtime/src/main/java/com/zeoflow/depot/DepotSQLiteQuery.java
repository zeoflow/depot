...
import com.zeoflow.sqlite.db.SupportSQLiteProgram;
...
public class DepotSQLiteQuery implements SupportSQLiteQuery, SupportSQLiteProgram {
    ...
    @SuppressWarnings("WeakerAccess")
    @VisibleForTesting
    static final TreeMap<Integer, DepotSQLiteQuery> sQueryPool = new TreeMap<>();

    /**
     * Copies the given SupportSQLiteQuery and converts it into DepotSQLiteQuery.
     *
     * @param supportSQLiteQuery The query to copy from
     * @return A new query copied from the provided one.
     */
    public static DepotSQLiteQuery copyFrom(SupportSQLiteQuery supportSQLiteQuery) {
        final DepotSQLiteQuery query = DepotSQLiteQuery.acquire(
    ...
    /**
     * Returns a new DepotSQLiteQuery that can accept the given number of arguments and holds the
     * given query.
     *
     * @param query         The query to prepare
     * @param argumentCount The number of query arguments
     * @return A DepotSQLiteQuery that holds the given query and has space for the given number of
     * arguments.
     */
    @SuppressWarnings("WeakerAccess")
    public static DepotSQLiteQuery acquire(String query, int argumentCount) {
        synchronized (sQueryPool) {
            final Map.Entry<Integer, DepotSQLiteQuery> entry =
                    sQueryPool.ceilingEntry(argumentCount);
            if (entry != null) {
                sQueryPool.remove(entry.getKey());
                final DepotSQLiteQuery sqliteQuery = entry.getValue();
                sqliteQuery.init(query, argumentCount);
                return sqliteQuery;
            }
        }
        DepotSQLiteQuery sqLiteQuery = new DepotSQLiteQuery(argumentCount);
        sqLiteQuery.init(query, argumentCount);
        return sqLiteQuery;
    }

    private DepotSQLiteQuery(int capacity) {
        mCapacity = capacity;
    ...
    /**
     * Copies arguments from another DepotSQLiteQuery into this query.
     *
     * @param other The other query, which holds the arguments to be copied.
     */
    public void copyArgumentsFrom(DepotSQLiteQuery other) {
