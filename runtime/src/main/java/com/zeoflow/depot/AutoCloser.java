...
final class AutoCloser {
            ...
            synchronized (mLock) {
                ...
                if (mOnAutoCloseCallback != null) {
                    ...
                    throw new IllegalStateException("mOnAutoCloseCallback is null but it should"
                            ...
                            + "against Depot at: https://issuetracker.google"
    ...
    /**
     * Since we need to construct the AutoCloser in the DepotDatabase.Builder, we need to set the
    ...
    public void init(@NonNull SupportSQLiteOpenHelper delegateOpenHelper) {
        if (mDelegateOpenHelper != null) {
            Log.e(Depot.LOG_TAG, "AutoCloser initialized multiple times. Please file a bug against"
                    + " depot at: https://issuetracker.google"
                    + ".com/issues/new?component=413107&template=1096568");
            return;
        }
