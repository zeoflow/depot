...
public class DBUtil {
    ...
    @Deprecated
    @NonNull
    public static Cursor query(DepotDatabase db, SupportSQLiteQuery sqLiteQuery, boolean maybeCopy) {
     ...
     */
    @NonNull
    public static Cursor query(@NonNull DepotDatabase db, @NonNull SupportSQLiteQuery sqLiteQuery,
    ...
    /**
     * Drops all FTS content sync triggers created by Depot.
     * <p>
     * FTS content sync triggers created by Depot are those that are found in the sqlite_master table
     * who's names start with 'depot_fts_content_sync_'.
     *
    ...
    public static void dropFtsSyncTriggers(SupportSQLiteDatabase db) {
        ...
        for (String triggerName : existingTriggers) {
            if (triggerName.startsWith("depot_fts_content_sync_")) {
