...
public class DepotOpenHelper extends SupportSQLiteOpenHelper.Callback {
    @Nullable
    ...
    /**
     * Depot v1 had a bug where the hash was not consistent if fields are reordered.
     * The new has fixes it but we still need to accept the legacy hash.
     */
    @NonNull // b/64290754
    private final String mLegacyHash;

    public DepotOpenHelper(@NonNull DatabaseConfiguration configuration, @NonNull Delegate delegate,
        ...
        if (!migrated) {
            if (mConfiguration != null
                ...
                throw new IllegalStateException("A migration from " + oldVersion + " to "
                        ...
                        + "DepotDatabase.Builder.addMigration(Migration ...) or allow for "
                        + "destructive migrations via one of the "
                        + "DepotDatabase.Builder.fallbackToDestructiveMigration* methods.");
            }
        }
    }
    ...
    private void checkIdentity(SupportSQLiteDatabase db) {
        if (hasDepotMasterTable(db)) {
            String identityHash = null;
            Cursor cursor = db.query(new SimpleSQLiteQuery(DepotMasterTable.READ_QUERY));
            ...
            if (!mIdentityHash.equals(identityHash) && !mLegacyHash.equals(identityHash)) {
                throw new IllegalStateException("Depot cannot verify the data integrity. Looks like"
                        + " you've changed schema but forgot to update the version number. You can"
                        + " simply fix this by increasing the version number.");
            }
        } else {
            // No depot_master_table, this might an a pre-populated DB, we must validate to see if
    ...
    private void updateIdentity(SupportSQLiteDatabase db) {
        ...
        db.execSQL(DepotMasterTable.createInsertQuery(mIdentityHash));
    }

    private void createMasterTableIfNotExists(SupportSQLiteDatabase db) {
        db.execSQL(DepotMasterTable.CREATE_QUERY);
    }

    private static boolean hasDepotMasterTable(SupportSQLiteDatabase db) {
        Cursor cursor = db.query("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name='"
                + DepotMasterTable.TABLE_NAME + "'");
        //noinspection TryFinallyCanBeTryWithResources
        try {
