...
// * Before each beginTransaction, DepotDatabase invokes InvalidationTracker to sync trigger states.
// * After each endTransaction, DepotDatabase invokes InvalidationTracker to refresh invalidated
// tables.
...
public class InvalidationTracker {
    ...
    private static final String UPDATE_TABLE_NAME = "depot_table_modification_log";
    ...
    public InvalidationTracker(DepotDatabase database, String... tableNames) {
    ...
    @SuppressWarnings("WeakerAccess")
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP_PREFIX)
    public InvalidationTracker(DepotDatabase database, Map<String, String> shadowTablesMap,
    ...
    void internalInit(SupportSQLiteDatabase database) {
        synchronized (this) {
            if (mInitialized) {
                Log.e(Depot.LOG_TAG, "Invalidation tracker is initialized twice :/.");
                return;
            }
    ...
    private static void appendTriggerName(StringBuilder builder, String tableName,
            String triggerType) {
        builder.append("`")
                .append("depot_table_modification_trigger_")
                .append(tableName)
                .append("_")
                .append(triggerType)
                .append("`");
    ...
    boolean ensureInitialization() {
        ...
        if (!mInitialized) {
            Log.e(Depot.LOG_TAG, "database is not initialized even though it is open");
            return false;
        }
        return true;
    }
    ...
    Runnable mRefreshRunnable = new Runnable() {
        ...
        public void run() {
            ...
            try {
                ...
                // This transaction has to be on the underlying DB rather than the DepotDatabase
                ...
                Log.e(Depot.LOG_TAG, "Cannot run invalidation tracker. Is the db closed?",
                        exception);
            } finally {
                closeLock.unlock();
     ...
     * <p>
     * This method is automatically called when {@link DepotDatabase#endTransaction()} is called but
    ...
    public void refreshVersionsAsync() {
        ...
        if (mPendingRefresh.compareAndSet(false, true)) {
            if (mAutoCloser != null) {
                ...
                // DepotDatabase, so the db can't be closed here, but we need to be sure that our
    ...
    /**
     * Called by DepotDatabase before each beginTransaction call.
     * <p>
