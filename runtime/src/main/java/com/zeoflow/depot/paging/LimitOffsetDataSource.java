...
package com.zeoflow.depot.paging;
...
import com.zeoflow.depot.DepotSQLiteQuery;
import com.zeoflow.depot.InvalidationTracker;
...
public abstract class LimitOffsetDataSource<T> extends androidx.paging.PositionalDataSource<T> {
    private final DepotSQLiteQuery mSourceQuery;
    private final String mCountQuery;
    private final String mLimitOffsetQuery;
    private final DepotDatabase mDb;
    @SuppressWarnings("FieldCanBeLocal")
    private final InvalidationTracker.Observer mObserver;
    private final boolean mInTransaction;
    private final AtomicBoolean mRegisteredObserver = new AtomicBoolean(false);

    protected LimitOffsetDataSource(@NonNull DepotDatabase db,
            @NonNull SupportSQLiteQuery query,
            boolean inTransaction,
            @NonNull
                    String... tables) {
        this(db, DepotSQLiteQuery.copyFrom(query), inTransaction, tables);
    }

    protected LimitOffsetDataSource(
            @NonNull DepotDatabase db,
            @NonNull SupportSQLiteQuery query,
            boolean inTransaction,
            boolean registerObserverImmediately,
            @NonNull String... tables) {
        this(db, DepotSQLiteQuery.copyFrom(query), inTransaction, registerObserverImmediately,
                tables);
    }

    protected LimitOffsetDataSource(
            @NonNull DepotDatabase db,
            @NonNull DepotSQLiteQuery query,
            boolean inTransaction,
            @NonNull String... tables) {
    ...
    public int countItems() {
        registerObserverIfNecessary();
        final DepotSQLiteQuery sqLiteQuery = DepotSQLiteQuery.acquire(mCountQuery,
    ...
    public void loadInitial(@NonNull LoadInitialParams params,
        ...
        DepotSQLiteQuery sqLiteQuery = null;
        Cursor cursor = null;
    ...
    public List<T> loadRange(int startPosition, int loadCount) {
        final DepotSQLiteQuery sqLiteQuery = getSQLiteQuery(startPosition, loadCount);
        if (mInTransaction) {
            mDb.beginTransaction();
            Cursor cursor = null;
    ...
    private DepotSQLiteQuery getSQLiteQuery(int startPosition, int loadCount) {
        final DepotSQLiteQuery sqLiteQuery = DepotSQLiteQuery.acquire(mLimitOffsetQuery,
