...
import com.zeoflow.depot.migration.Migration;
import com.zeoflow.depot.util.SneakyThrow;
import com.zeoflow.sqlite.db.SimpleSQLiteQuery;
import com.zeoflow.sqlite.db.SupportSQLiteDatabase;
import com.zeoflow.sqlite.db.SupportSQLiteOpenHelper;
import com.zeoflow.sqlite.db.SupportSQLiteQuery;
import com.zeoflow.sqlite.db.SupportSQLiteStatement;
import com.zeoflow.sqlite.db.framework.FrameworkSQLiteOpenHelperFactory;
...
/**
 * Base class for all Depot databases. All classes that are annotated with {@link Database} must
 * extend this class.
 * <p>
 * DepotDatabase provides direct access to the underlying database implementation but you should
 * prefer using {@link Dao} classes.
 *
 * @see Database
 */
public abstract class DepotDatabase {
    ...
    /**
     * Creates a DepotDatabase.
     * <p>
     * You cannot create an instance of a database, instead, you should acquire it via
     * {@link Depot#databaseBuilder(Class, String)},
     * {@link Depot#databaseBuilder(Context, Class, String)} or
     * {@link Depot#inMemoryDatabaseBuilder(Context, Class)}.
     */
    ...
    /**
     * Called by {@link Depot} when it is initialized.
     *
    ...
    public void init(@NonNull DatabaseConfiguration configuration) {
        ...
        AutoClosingDepotOpenHelper autoClosingDepotOpenHelper =
                unwrapOpenHelper(AutoClosingDepotOpenHelper.class, mOpenHelper);

        if (autoClosingDepotOpenHelper != null) {
            mAutoCloser = autoClosingDepotOpenHelper.getAutoCloser();
     ...
     * method.
     * Note that this method is called when the DepotDatabase is initialized.
     *
     * @param config The configuration of the Depot database.
    ...
    /**
     * Called when the DepotDatabase is created.
     * <p>
     ...
     * <p>
     * After deleting the rows, Depot will set a WAL checkpoint and run VACUUM. This means that the
     ...
     * @return A Cursor obtained by running the given query in the Depot database.
     */
    @NonNull
     ...
     * <p>
     * Depot will only perform at most one transaction at a time.
     *
     ...
     *
     * @see DepotDatabase.Builder#setJournalMode(JournalMode)
     */
    public enum JournalMode {

        /**
         * Let Depot choose the journal mode. This is the default value when no explicit value is
         * specified.
         * <p>
    ...
    /**
     * Builder for DepotDatabase.
     *
     * @param <T> The type of the abstract database class.
     */
    public static class Builder<T extends DepotDatabase> {
        ...
        /**
         * Configures Depot to create and open the database using a pre-packaged database located in
         * the application 'assets/' folder.
         * <p>
         * Depot does not open the pre-packaged database, instead it copies it into the internal
        ...
        /**
         * Configures Depot to create and open the database using a pre-packaged database file.
         * <p>
         * Depot does not open the pre-packaged database, instead it copies it into the internal
         * app database folder and then opens it. The given file must be accessible and the right
         * permissions must be granted for Depot to copy the file.
         * <p>
         * The pre-packaged database schema will be validated. It might be best to create your
         * pre-packaged database schema utilizing the exported schema files generated when
         * {@link Database#exportSchema()} is enabled.
         ...
         * that the pre-packaged database was successfully opened by Depot and can be cleaned up.
         * <p>
         * This method is not supported for an in memory database {@link Builder}.
         *
        ...
        /**
         * Configures Depot to create and open the database using a pre-packaged database via an
         * {@link InputStream}.
         * <p>
         * This is useful for processing compressed database files. Depot does not open the
         * pre-packaged database, instead it copies it into the internal app database folder, and
         * then open it. The {@link InputStream} will be closed once Depot is done consuming it.
         * <p>
         * The pre-packaged database schema will be validated. It might be best to create your
         * pre-packaged database schema utilizing the exported schema files generated when
         * {@link Database#exportSchema()} is enabled.
         ...
         *                            callable is only invoked if Depot needs to create and open the
         ...
         * <p>
         * Each Migration has a start and end versions and Depot runs these migrations to bring the
         * database to the latest version.
         * <p>
         * If a migration item is missing between current version and the latest version, Depot
         ...
         * going version 3 to 5 without going to version 4). If Depot opens a database at version
         * 3 and latest version is &gt;= 5, Depot will use the migration object that can migrate from
        ...
        /**
         * Disables the main thread query check for Depot.
         * <p>
         * Depot ensures that Database is never accessed on the main thread because it may lock the
         ...
         * {@link DepotDatabase} for a single SQLite database file.
         * <p>
         ...
         * possibility of a deadlock. Depot will not use more than one thread at a time from this
        ...
        /**
         * Sets whether table invalidation in this instance of {@link DepotDatabase} should be
         * broadcast and synchronized with other instances of the same {@link DepotDatabase},
        ...
        /**
         * Allows Depot to destructively recreate database tables if {@link Migration}s that would
         * migrate old database schemas to the latest schema version are not found.
         * <p>
         * When the database version on the device does not match the latest schema version, Depot
         ...
         * crashing.
         * <p>
         * If the database was create from an asset or a file then Depot will try to use the same
         * file to re-create the database, otherwise this will delete all of the data in the
         * database tables managed by Depot.
         * <p>
         * To let Depot fallback to destructive migration only during a schema downgrade then use
        ...
        /**
         * Allows Depot to destructively recreate database tables if {@link Migration}s are not
        ...
        /**
         * Informs Depot that it is allowed to destructively recreate database tables from specific
         ...
         *
         * @param startVersions The set of schema versions from which Depot should use a destructive
         *                      migration.
         * @return This {@link Builder} instance.
         */
        @NonNull
         ...
         * {@link ProvidedTypeConverter} otherwise Depot will throw an exception.
         * @return This {@link Builder} instance.
         */
        @NonNull
         ...
         * {@link DepotDatabase.Callback#onOpen callback}.
         * <p>
         * All configuration should happen in your {@link DepotDatabase.Callback#onOpen}
         * callback so it is re-applied every time the database is re-opened. Note that the
         * {@link DepotDatabase.Callback#onOpen} will be called every time the database is re-opened.
         * <p>
         * The auto-closing database operation runs on the query executor.
         * <p>
         * The database will not be reopened if the DepotDatabase or the
         * SupportSqliteOpenHelper is closed manually (by calling
         * {@link DepotDatabase#close()} or {@link SupportSQLiteOpenHelper#close()}. If the
         * database is closed manually, you must create a new database using
         * {@link DepotDatabase.Builder#build()}.
         *
        ...
        @NonNull
        @SuppressWarnings("MissingGetterMatchingBuilder")
        @ExperimentalDepotApi // When experimental is removed, add these parameters to
         ...
         * <p>
         * By default, all DepotDatabases use in memory storage for TEMP tables and enables recursive
         * triggers.
         *
        ...
        public T build() {
            ...
            if (mAutoCloseTimeout > 0) {
                ...
                factory = new AutoClosingDepotOpenHelperFactory(factory, autoCloser);
            }
            ...
            DatabaseConfiguration configuration =
                    new DatabaseConfiguration(
                            ...
                            mTypeConverters,
                            mAutoMigrationSpecs);
            T db = Depot.getGeneratedImplementation(mDatabaseClass, DB_IMPL_SUFFIX);
    ...
    public static class MigrationContainer {
        ...
        private void addMigration(Migration migration) {
            ...
            if (existing != null) {
                Log.w(Depot.LOG_TAG, "Overriding migration " + existing + " with " + migration);
            }
    ...
    /**
     * Callback for {@link DepotDatabase}.
     */
     ...
     * <p>
     * This callback will be invoked after the pre-package DB is copied but before Depot had
     * a chance to open it and therefore before the {@link DepotDatabase.Callback} methods are
     * invoked. This callback can be useful for updating the pre-package DB schema to satisfy
     * Depot's schema validation.
     */
     ...
     *
     * @see DepotDatabase.Builder#setQueryCallback
     */
