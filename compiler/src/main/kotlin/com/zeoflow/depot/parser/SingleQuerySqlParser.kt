...
object SingleQuerySqlParser {
    fun <T> parse(
        input: String,
        visit: (statement: com.zeoflow.depot.parser.SQLiteParser.Sql_stmtContext, syntaxErrors: MutableList<String>) -> T,
        fallback: (syntaxErrors: List<String>) -> T
    ): T {
        val inputStream = CharStreams.fromString(input)
        val lexer = com.zeoflow.depot.parser.SQLiteLexer(inputStream)
        val tokenStream = CommonTokenStream(lexer)
        val parser = com.zeoflow.depot.parser.SQLiteParser(tokenStream)
        ...
        try {
            ...
            val statements = statementList.first().children
                .filterIsInstance<com.zeoflow.depot.parser.SQLiteParser.Sql_stmtContext>()
