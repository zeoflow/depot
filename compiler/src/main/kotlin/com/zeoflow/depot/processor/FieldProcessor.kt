...
class FieldProcessor(
    ...
    fun process(): Field {
        ...
        val columnInfo = element.getAnnotation(com.zeoflow.depot.ColumnInfo::class)?.value
        val name = element.name
        val rawCName = if (columnInfo != null && columnInfo.name != com.zeoflow.depot.ColumnInfo.INHERIT_FIELD_NAME) {
            columnInfo.name
        } else {
            name
        }
    ...
    private fun extractDefaultValue(
        ...
        val defaultValue = if (affinity == SQLTypeAffinity.TEXT) {
            if (value == com.zeoflow.depot.ColumnInfo.VALUE_UNSPECIFIED) {
                null
            ...
            if (value == com.zeoflow.depot.ColumnInfo.VALUE_UNSPECIFIED || trimmed == "") {
                null
            } else {
                value
            }
